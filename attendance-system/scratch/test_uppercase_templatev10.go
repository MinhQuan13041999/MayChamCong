...
func main() {
	...
	formats := []string{
		// 1. TEMPLATEV10 uppercase + PIN + FID + TMP
		fmt.Sprintf("DATA UPDATE TEMPLATEV10 PIN=1015\tFID=6\tSize=1616\tValid=1\tTMP=%s", realTemplate),
		// 2. TEMPLATEV10 uppercase + PIN + FingerID + Template
		fmt.Sprintf("DATA UPDATE TEMPLATEV10 PIN=1015\tFingerID=6\tSize=1616\tVal=1\tTemplate=%s", realTemplate),
		// 3. FINGERTEMPLATE uppercase + PIN + FingerID + Template (using real template)
		fmt.Sprintf("DATA UPDATE FINGERTEMPLATE PIN=1015\tFingerID=6\tSize=1616\tVal=1\tTemplate=%s", realTemplate),
		// 4. BIODATA uppercase + PIN + FID + TMP
		fmt.Sprintf("DATA UPDATE BIODATA PIN=1015\tFID=6\tSize=1616\tValid=1\tTMP=%s", realTemplate),
	}
