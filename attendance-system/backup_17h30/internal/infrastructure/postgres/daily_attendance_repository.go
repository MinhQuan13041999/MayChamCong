...
func (r *DailyAttendanceRepository) Upsert(ctx context.Context, da *entity.DailyAttendance) error {
	query := `
		INSERT INTO daily_attendance (employee_id, date, shift_id, first_in, last_out, late_minutes, early_minutes, working_hours, attendance_status, overtime_minutes, leave_id, updated_at)
...
func (r *DailyAttendanceRepository) Query(ctx context.Context, employeeID string, from, to time.Time) ([]entity.DailyAttendance, error) {
	query := `
		SELECT id, employee_id, date, shift_id, first_in, last_out, late_minutes, early_minutes, working_hours, attendance_status, overtime_minutes, leave_id, created_at, updated_at
	...
	for rows.Next() {
		...
		if err := rows.Scan(&da.ID, &da.EmployeeID, &da.Date, &shiftID, &firstIn, &lastOut,
			&da.LateMinutes, &da.EarlyMinutes, &da.WorkingHours, &da.AttendanceStatus, &da.OvertimeMinutes, &leaveID, &da.CreatedAt, &da.UpdatedAt); err != nil {
			return nil, err
		}
		da.ShiftID = shiftID
		da.LeaveID = leaveID
