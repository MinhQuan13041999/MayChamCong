...
## 14. Cấu hình thông báo Email và Zalo

Hệ thống gửi thông báo cho log mới nhận trong vòng 10 phút, vì vậy lần đồng bộ dữ liệu lịch sử đầu tiên sẽ không gửi hàng loạt tin cũ. Cảnh báo quên check-out chạy mỗi phút và chỉ gửi khi nhân viên đã có check-in nhưng chưa có check-out sau giờ kết thúc ca 30 phút.

Điền cấu hình trong `config.yaml`:
...
notifications:
  ...
  instant_max_age_minutes: 10
  checkout_grace_minutes: 30
```

- Với Gmail, dùng App Password thay cho mật khẩu đăng nhập thông thường.
- Với Zalo, cần Zalo Official Account, access token hợp lệ và nhập **Zalo User ID** vào hồ sơ từng nhân viên. Số điện thoại không thể thay thế trực tiếp cho Zalo User ID trong API OA.
- Sau khi đổi cấu hình, khởi động lại backend.
