...
// SetNotificationService gắn bộ gửi thông báo cho log ADMS mới.
func (s *ADMSService) SetNotificationService(notifier *NotificationService) {
	s.notifier = notifier
}
...
func (s *ADMSService) ConfirmCommand(ctx context.Context, sn string, body string) (string, error) {
	...
	for _, line := range lines {
		...
		if err == nil && cmd != nil {
			...
			if strings.HasPrefix(upperCmd, "ENROLL_INFO ") || strings.HasPrefix(upperCmd, "ENROLL_FP ") {
				// A non-zero reply to an interactive enrollment command also covers a
				// user pressing ESC on the terminal.  Retrying with another enrollment
				// command traps the user in the enrollment screen, so treat it as a
				// cancelled/failed request and require an explicit new request from web.
				zap.L().Info("[ADMS] Enrollment command cancelled or rejected; not retrying",
					zap.String("serial", sn),
					zap.Int64("cmd_id", cmdID),
					zap.Int("return_code", *retCode),
				)
...
func extractPinFromADMSCommand(command string) string {
	...
	if !strings.HasPrefix(upper, "ENROLL_INFO") && !strings.HasPrefix(upper, "ENROLL_FP") {
		return ""
	}
...
func admsFieldValue(field string) string {
...
func (s *ADMSService) parseAndSaveAttLogs(ctx context.Context, deviceID string, deviceName string, body string) (int, error) {
	...
	for _, line := range lines {
		...
		pin := admsFieldValue(parts[0])
		timeStr := admsFieldValue(parts[1])
		statusIndex := 2
		verifyIndex := 3
		// Some firmware sends "PIN<TAB>123<TAB>date time..." while
		// others sends "PIN=123<TAB>date time...". Handle both forms.
		if strings.EqualFold(strings.TrimSpace(parts[0]), "PIN") && len(parts) >= 3 {
			pin = admsFieldValue(parts[1])
			timeStr = admsFieldValue(parts[2])
			statusIndex = 3
			verifyIndex = 4
		}
		// A space-delimited payload splits the date and time into two fields.
		if !strings.Contains(line, "\t") && len(parts) >= 3 && !strings.Contains(timeStr, " ") {
		...
		status := "0"
		if len(parts) > statusIndex {
			status = admsFieldValue(parts[statusIndex])
		}

		verify := "1"
		if len(parts) > verifyIndex {
			verify = admsFieldValue(parts[verifyIndex])
		}
		...
		if status == "1" {
			...
			checkType = entity.CheckTypeIn // OT vào
		} else if status == "5" {
			checkType = entity.CheckTypeOut // OT ra
		}
	...
	if s.notifier != nil {
		newLogs = filterNewAttendanceLogs(ctx, s.attendanceRepo, logs)
	}
	...
	// Chỉ lưu log thô khi nhận dữ liệu từ thiết bị. Tính công hằng ngày
	// được thực hiện riêng khi người dùng nhấn nút "Tính công".
...
func parseADMSBiometricPayload(raw string) *admsBiometricPayload {
	...
	// A missing finger index must never silently become index 0.  That would
	// overwrite a valid thumb template and makes a later push unrecoverable.
	payload := &admsBiometricPayload{FingerIndex: -1}
	backupNo := -1
	...
	for _, part := range parts {
		...
		case "pin":
			payload.Pin = value
		case "fingerid", "fid", "index":
			if val, err := strconv.Atoi(value); err == nil {
				payload.FingerIndex = val
			}
		case "no":
			// BIODATA uses No as a biometric-record number and Index as the
			// actual finger.  Some older terminals send only No, so keep it
			// as a fallback instead of letting it override Index.
			if val, err := strconv.Atoi(value); err == nil {
				backupNo = val
			}
	...
	if payload.FingerIndex < 0 {
		payload.FingerIndex = backupNo
	}
	// Several ADMS firmwares omit Size.  The FINGERTEMPLATE fallback requires
	// it, so derive the byte size from a base64 template where possible.
	if payload.TemplateSize == 0 && payload.TemplateData != "" {
		if decoded, err := base64.StdEncoding.DecodeString(payload.TemplateData); err == nil {
			payload.TemplateSize = len(decoded)
		} else {
			payload.TemplateSize = len(payload.TemplateData)
		}
	}
			...
			if err := s.fingerprintRepo.Upsert(ctx, fp); err != nil {
				...
				go func(eID string, srcDevID string) {
					...
					for _, d := range devices {
						...
						for _, f := range fps {
							fpCmd := buildADMSFingerprintCommand(pin, f.FingerIndex, f.TemplateSize, f.TemplateData)
