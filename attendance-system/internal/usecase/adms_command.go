...
func buildADMSUserCommandVariants(pin, fullName, cardNo string) []string {
	...
	return []string{
		fmt.Sprintf("DATA UPDATE USERINFO PIN=%s\tName=%s\tPri=0\tPasswd=\tCard=%s\tGrp=1", normalizedPin, fullName, cardValue),
		fmt.Sprintf("DATA UPDATE USERINFO Pin=%s\tName=%s\tPri=0\tPasswd=\tCard=%s\tGrp=1", normalizedPin, fullName, cardValue),
		fmt.Sprintf("DATA UPDATE USER PIN=%s\tName=%s\tPri=0\tPasswd=\tCard=%s\tGrp=1", normalizedPin, fullName, cardValue),
		fmt.Sprintf("DATA UPDATE USER Pin=%s\tName=%s\tPri=0\tPasswd=\tCard=%s\tGrp=1", normalizedPin, fullName, cardValue),
		fmt.Sprintf("DATA UPDATE user PIN=%s\tName=%s\tPri=0\tPasswd=\tCard=%s\tGrp=1", normalizedPin, fullName, cardValue),
		fmt.Sprintf("DATA UPDATE USERINFO PIN=%s\tNAME=%s\tPRI=0\tPASSWD=\tCARD=%s\tGRP=1", normalizedPin, fullName, cardValue),
	}
...
func buildADMSEnrollCommand(pin string, fingerIndex int) string {
	// This terminal returns -1002 for ENROLL_INFO. ENROLL_FP is the compatible
	// ADMS command for its remote fingerprint-enrollment protocol.
	return buildADMSFallbackEnrollCommand(pin, fingerIndex)
}

func buildADMSFallbackEnrollCommand(pin string, fingerIndex int) string {
	normalizedPin := normalizeADMSPin(pin)
	return fmt.Sprintf("ENROLL_FP PIN=%s\tFID=%d\tRETRY=3\tOVERWRITE=1", admsEscapeValue(normalizedPin), fingerIndex)
}
