...
type AttendanceLog struct {
	...
	WorkSegment       string     `json:"work_segment"`
...
// RotationPattern đại diện cho chu kỳ xoay ca
type RotationPattern struct {
	ID              string    `json:"id"`
	Name            string    `json:"name"`
	PatternSequence string    `json:"pattern_sequence"` // Chuỗi JSON chứa array [{shift_id: string, duration: int}]
	CreatedAt       time.Time `json:"created_at"`
}

// ShiftSwapRequest đại diện cho yêu cầu đổi ca giữa 2 nhân viên
type ShiftSwapRequest struct {
	...
	CreatedAt              time.Time  `json:"created_at"`
	RequestingEmployeeName string     `json:"requesting_employee_name,omitempty"`
...
type DailyAttendance struct {
	...
	RegularWorkingMinutes int        `json:"regular_working_minutes"`
...
// EmployeeFace đại diện cho mẫu khuôn mặt đã đăng ký của nhân viên
type EmployeeFace struct {
	ID             int64     `json:"id"`
	EmployeeID     string    `json:"employee_id"`
	FaceDescriptor string    `json:"face_descriptor"` // JSON string representation of 128 float array
	CreatedAt      time.Time `json:"created_at"`
}
