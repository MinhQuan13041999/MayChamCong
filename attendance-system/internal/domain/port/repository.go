...
type AttendanceLogRepository interface {
	...
	UpdateValidity(ctx context.Context, id int64, isValid bool, reason string) error
	UpdateClassification(ctx context.Context, id int64, isValid bool, reason, workSegment string, overtimeRequestID *string) error
}
...
type EmployeeShiftRepository interface {
	...
	Delete(ctx context.Context, id string) error
	List(ctx context.Context) ([]entity.EmployeeShift, error)
}

// RotationPatternRepository quản lý chu kỳ xoay ca
type RotationPatternRepository interface {
	Create(ctx context.Context, rp *entity.RotationPattern) error
	List(ctx context.Context) ([]entity.RotationPattern, error)
	GetByID(ctx context.Context, id string) (*entity.RotationPattern, error)
	Update(ctx context.Context, rp *entity.RotationPattern) error
	Delete(ctx context.Context, id string) error
}

// ShiftSwapRequestRepository quản lý đơn đổi ca
type ShiftSwapRequestRepository interface {
	Create(ctx context.Context, ssr *entity.ShiftSwapRequest) error
	List(ctx context.Context) ([]entity.ShiftSwapRequest, error)
	GetByID(ctx context.Context, id string) (*entity.ShiftSwapRequest, error)
	UpdateStatus(ctx context.Context, id string, status string, approvedBy string) error
	GetApprovedSwapForEmployeeOnDate(ctx context.Context, employeeID string, date time.Time) (*entity.ShiftSwapRequest, error)
}
...
type OvertimeRequestRepository interface {
	...
	ListApprovedOTOnDate(ctx context.Context, employeeID string, date time.Time) ([]entity.OvertimeRequest, error)
}
...
// FaceRepository quản lý mẫu khuôn mặt của nhân viên
type FaceRepository interface {
	Upsert(ctx context.Context, employeeID string, faceDescriptor string) error
	GetByEmployee(ctx context.Context, employeeID string) (*entity.EmployeeFace, error)
	ListAll(ctx context.Context) ([]entity.EmployeeFace, error)
