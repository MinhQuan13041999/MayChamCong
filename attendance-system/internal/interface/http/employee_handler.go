...
func (h *EmployeeHandler) Routes(r chi.Router) {
	...
	r.Group(func(adminOnly chi.Router) {
		...
		// POST fallback for deployments behind Apache/IIS configurations that
		// reject collection-level DELETE requests before they reach Go.
		adminOnly.Post("/employees/delete-all", h.DeleteAll)
		adminOnly.Put("/employees/{id}", h.Update)
		adminOnly.Delete("/employees", h.DeleteAll)
...
func (h *EmployeeHandler) Create(w http.ResponseWriter, r *http.Request) {
	...
	if req.EnrollFingerprint {
		if req.DeviceID == "" || req.DeviceUserID == "" {
			writeError(w, http.StatusBadRequest, fmt.Errorf("device_id and device_user_id are required when enroll_fingerprint is enabled"))
			return
		}
		// Create the account through the common service first, then use the
		// normal device-sync and biometric flows.  This keeps enrollment working
		// for both ADMS terminals and standalone COM/SDK terminals.
		if err := h.service.CreateEmployee(r.Context(), e); err != nil {
			writeError(w, http.StatusBadRequest, err)
			return
		}
		if _, err := h.service.SyncEmployeeToDevice(r.Context(), e.ID, req.DeviceID, req.DeviceUserID); err != nil {
			writeError(w, http.StatusBadRequest, err)
			return
		}
		if h.biometricService == nil {
			writeError(w, http.StatusBadRequest, fmt.Errorf("biometric service is not configured"))
			return
		}
		if err := h.biometricService.EnrollFingerprint(r.Context(), e.ID, req.DeviceID, 0); err != nil {
			writeError(w, http.StatusBadRequest, err)
			return
		}
...
func (h *EmployeeHandler) DeleteAll(w http.ResponseWriter, r *http.Request) {
	var req struct {
		Confirmation string `json:"confirmation"`
	}
	if err := json.NewDecoder(r.Body).Decode(&req); err != nil || req.Confirmation != "DELETE_ALL_EMPLOYEES" {
		writeError(w, http.StatusBadRequest, fmt.Errorf("invalid delete-all confirmation"))
		return
	}

	deleted, err := h.service.DeleteAllEmployees(r.Context())
	if err != nil {
		writeError(w, http.StatusInternalServerError, err)
		return
	}

	_ = h.processor.CreateAuditLog(r.Context(), getUserID(r), "DELETE_ALL_EMPLOYEES", "employee", "all",
		fmt.Sprintf("Deleted all employee profiles (%d records)", deleted), r.RemoteAddr)
