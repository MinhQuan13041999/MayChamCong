...
func main() {
	...
	fmt.Println("Connected! Listening for Realtime Scan Events via GetRTLog (Looping for 45s)...")
	...
	for {
		select {
		case <-timeout:
			fmt.Println("Test completed after 45s.")
			return
		case <-ticker.C:
			...
			if resGetRT != nil && comResultTrue(resGetRT) {
				fmt.Printf("⚡ [%s] GetRTLog DETECTED EVENT! Reading General Log Data...\n", time.Now().Format("15:04:05.000"))
				...
				for {
					...
					fmt.Printf("   🎉 EVENT DATA: PIN '%s' at %04d-%02d-%02d %02d:%02d:%02d\n", pin, y, m, d, h, min, s)
				}
