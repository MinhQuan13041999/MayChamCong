...
func main() {
	...
	ipAddress := "192.168.1.100" // Thay đổi thành IP thật của máy chấm công của bạn
	...
	if res.VT == ole.VT_BOOL {
		...
		// Thỉnh thoảng VT là VT_I4 (integer) tương ứng với 1 (true) hoặc 0 (false)
	...
	if err == nil {
		...
		fmt.Printf("[2] Lỗi lấy Serial Number: %v\n", err)
	}
	vSN.Clear()
	...
	if readLogRes.Value().(bool) {
		...
		for {
			...
			if !resLoop.Value().(bool) {
				break
			}

			count++
			enrollNumber := vEnrollNumber.Value().(string)
			verifyMode := vVerifyMode.Value().(int32)
			inOutMode := vInOutMode.Value().(int32)
			year := vYear.Value().(int32)
			month := vMonth.Value().(int32)
			day := vDay.Value().(int32)
			hour := vHour.Value().(int32)
			minute := vMinute.Value().(int32)
			second := vSecond.Value().(int32)
