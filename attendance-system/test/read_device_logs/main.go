...
func main() {
	...
	if err != nil || resRead == nil || resRead.Val == 0 {
		log.Fatalf("ReadGeneralLogData failed: %v", err)
	}
	...
	for {
		...
		logLine := fmt.Sprintf("#%d | PIN: '%s' | LogTime: %04d-%02d-%02d %02d:%02d:%02d", count, enrollNumber, y, m, d, h, min, s)
		allLogs = append(allLogs, logLine)
	}

	fmt.Printf("Total logs retrieved from device: %d\n", count)
	start := 0
	if len(allLogs) > 20 {
		start = len(allLogs) - 20
	}
	fmt.Println("--- Recent Logs on Device ---")
	for i := start; i < len(allLogs); i++ {
		fmt.Println(allLogs[i])
	}
}
