...
func main() {
	...
	// Method A: SSR_GetGeneralLogData
	resReadA, _ := oleutil.CallMethod(zkem, "ReadGeneralLogData", dwMachineNumber)
	fmt.Printf("ReadGeneralLogData result: %v\n", resReadA.Value())
	...
	for {
		...
		fmt.Printf("  [SSR Log #%d] PIN '%s' at %04d-%02d-%02d %02d:%02d:%02d\n", countA, pin, y, m, d, h, min, s)
	}
	fmt.Printf("Total logs found via SSR_GetGeneralLogData: %d\n", countA)

	// Method B: ReadTimeGLogData (for newer TFT firmwares)
	resReadB, _ := oleutil.CallMethod(zkem, "ReadTimeGLogData", dwMachineNumber, "2026-01-01 00:00:00", "2026-12-31 23:59:59")
	fmt.Printf("ReadTimeGLogData result: %v\n", resReadB)
