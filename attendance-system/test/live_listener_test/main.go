...
func main() {
	...
	fmt.Println("Connected! Listening for live scans... Press Ctrl+C or wait 30 seconds.")
	...
	for {
		...
		case <-ticker.C:
			...
			for {
				...
				key := fmt.Sprintf("%s_%04d-%02d-%02d_%02d:%02d:%02d", pin, y, m, d, h, min, s)
				if !seen[key] {
					seen[key] = true
					fmt.Printf("⚡ [%s] NEW SCAN DETECTED! PIN: '%s' | Time: %04d-%02d-%02d %02d:%02d:%02d\n",
						time.Now().Format("15:04:05"), pin, y, m, d, h, min, s)
				}
			}
