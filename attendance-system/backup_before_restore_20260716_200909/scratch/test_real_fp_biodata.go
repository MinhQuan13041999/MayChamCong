...
func main() {
	...
	formats := []string{
		// Format 1: BIODATA uppercase + PIN + Tmp (standard ADMS 2.0)
		fmt.Sprintf("DATA UPDATE BIODATA PIN=1015\tNo=1\tIndex=6\tValid=1\tDuress=0\tType=9\tMajorVer=5\tMinorVer=8\tFormat=0\tTmp=%s", realTemplate),
		// Format 2: biodata lowercase + PIN + Tmp
		fmt.Sprintf("DATA UPDATE biodata PIN=1015\tNo=1\tIndex=6\tValid=1\tDuress=0\tType=9\tMajorVer=5\tMinorVer=8\tFormat=0\tTmp=%s", realTemplate),
		// Format 3: FINGERTEMPLATE + PIN + FID + TMP (matching how the device pushes)
		fmt.Sprintf("DATA UPDATE FINGERTEMPLATE PIN=1015\tFID=6\tSize=1616\tValid=1\tTMP=%s", realTemplate),
		// Format 4: fingertemplate + PIN + FID + TMP
		fmt.Sprintf("DATA UPDATE fingertemplate PIN=1015\tFID=6\tSize=1616\tValid=1\tTMP=%s", realTemplate),
		// Format 5: templatev10 + PIN + FID + TMP
