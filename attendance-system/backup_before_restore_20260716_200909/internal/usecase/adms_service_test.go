...
func TestBuildADMSFallbackEnrollCommand(t *testing.T) {
	got := buildADMSFallbackEnrollCommand("321")
...
func TestIsADMSDeviceUsesSerialWhenEnabledFlagIsFalse(t *testing.T) {
	d := &entity.Device{SerialNumberADMS: "8116255100515"}
	if !isADMSDevice(d) {
		t.Fatal("expected device with ADMS serial to be treated as ADMS")
	}
}
...
func TestBuildADMSUserCommandUsesCompatibleFormat(t *testing.T) {
	...
	want := "DATA UPDATE USERINFO PIN=001\tNAME=Nguyen Van An\tPRI=0"
	if got != want {
...
func TestBuildADMSEnrollCommandUsesFingerprintEnrollmentVariant(t *testing.T) {
	got := buildADMSEnrollCommand("EMP001")
