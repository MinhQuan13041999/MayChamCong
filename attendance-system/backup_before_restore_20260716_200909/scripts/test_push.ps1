...
if (-not $DeviceId) { Write-Error "Usage: .\test_push.ps1 -DeviceId <id> [-Token <jwt>]"; exit 1 }
$headers = @{}
if ($Token) { $headers.Add('Authorization', "Bearer $Token") }
Write-Host "POST /api/v1/devices/$DeviceId/sync-employees"
try {
    $r = Invoke-RestMethod -Method Post -Uri "http://localhost:8085/api/v1/devices/$DeviceId/sync-employees" -Headers $headers -Body '{}' -ContentType 'application/json' -ErrorAction Stop
